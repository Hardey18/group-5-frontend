...
import {
  ...
  LinearProgress, Tooltip, IconButton, Table, TableBody,
  TableCell, TableContainer, TableHead, TableRow,
  Stack, CircularProgress, Alert, Fade,
} from "@mui/material";
import {
...
interface Prediction {
  id: number; customerId: number; avgDataUsage: number;
  usageChangePct: number; rechargeFreq: number; rechargeChangePct: number;
  engagementScore: number; networkQuality: number; hasComplaint: number;
  complaintText: string; churn: number; churnRiskScore: number;
  customerType: string; timestamp: string;
}
...
function StatCard({ icon, label, value, sub, accent, pulse }: {
  ...
  return (
    <Card sx={{
      bgcolor: M.surface, border: `1px solid ${M.border}`,
      borderTop: `3px solid ${accent}`, borderRadius: "14px",
      boxShadow: M.shadow, position: "relative", overflow: "hidden",
      transition: "transform 0.2s, box-shadow 0.2s",
      "&:hover": { transform: "translateY(-3px)", boxShadow: M.shadowMd },
    }}>
      <Box sx={{ position: "absolute", top: -20, right: -20, width: 90, height: 90, borderRadius: "50%", bgcolor: `${accent}12`, pointerEvents: "none" }} />
      <CardContent sx={{ p: "20px 24px" }}>
        <Stack direction="row" alignItems="flex-start" justifyContent="space-between">
          <Box>
            <Typography sx={{ color: M.textMuted, fontSize: "0.65rem", fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", mb: 0.75 }}>
              {label}
            </Typography>
            <Typography sx={{ color: M.text, fontSize: "2rem", fontWeight: 800, lineHeight: 1, letterSpacing: "-0.02em" }}>
              {value}
            </Typography>
            {sub && <Typography sx={{ color: M.textMuted, fontSize: "0.72rem", mt: 0.5 }}>{sub}</Typography>}
          </Box>
          <Avatar sx={{ bgcolor: `${accent}18`, border: `1.5px solid ${accent}40`, width: 46, height: 46, position: "relative" }}>
            {pulse && (
              <Box sx={{
                position: "absolute", inset: 0, borderRadius: "50%",
                border: `2px solid ${accent}`,
                animation: "pr 2s cubic-bezier(0.4,0,0.6,1) infinite",
                "@keyframes pr": { "0%": { transform: "scale(1)", opacity: 0.6 }, "100%": { transform: "scale(1.6)", opacity: 0 } },
              }} />
            )}
            <Box sx={{ color: accent, "& svg": { fontSize: "1.3rem" } }}>{icon}</Box>
          </Avatar>
        </Stack>
      </CardContent>
    </Card>
...
function ChartCard({ title, children, height = 280 }: { title: string; children: React.ReactNode; height?: number }) {
  return (
    <Card sx={{ bgcolor: M.surface, border: `1px solid ${M.border}`, borderRadius: "14px", p: 3, height: "100%", boxShadow: M.shadow }}>
      <Typography sx={{ color: M.textMuted, fontSize: "0.65rem", fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", mb: 2.5 }}>
        {title}
      </Typography>
...
export default function ChurnDashboard() {
  ...
  const { data, isLoading, isError, dataUpdatedAt, refetch, isFetching } = useDashboardData(1, 100);

  const predictions: Prediction[] = useMemo(() => data?.predictions ?? [], [data]);

  const filtered = useMemo(() => predictions.filter((p) => {
    const segOk = segment === "All Customers" ? true : p.customerType === segment;
    const risk = getRiskLevel(p.churnRiskScore);
    const riskOk = riskFilter === "All risk levels" ? true
      : riskFilter === "Low (< 0.3)" ? risk === "low"
      : riskFilter === "Medium (0.3 - 0.6)" ? risk === "medium"
      : risk === "high";
    return segOk && riskOk;
  ...
  const a = useMemo(() => {
    ...
    const segmentDist = ["Healthy", "Silent Churn", "Vocal Churn"].map(seg => ({
      name: seg, value: filtered.filter(p => p.customerType === seg).length, color: SEGMENT_COLORS[seg],
    }));

    const riskDist = [
      { name: "Low", value: filtered.filter(p => getRiskLevel(p.churnRiskScore) === "low").length, color: M.success },
      { name: "Medium", value: filtered.filter(p => getRiskLevel(p.churnRiskScore) === "medium").length, color: M.warning },
      { name: "High", value: filtered.filter(p => getRiskLevel(p.churnRiskScore) === "high").length, color: M.danger },
    ];

    const aiActionDist = [
      { name: "Reactive", value: reactive, color: M.danger },
      { name: "Proactive", value: proactive, color: M.warning },
      { name: "Monitor", value: total - reactive - proactive, color: M.info },
    ];

    const scatterData = filtered.map(p => ({
      x: parseFloat(p.engagementScore.toFixed(3)),
      y: parseFloat(p.churnRiskScore.toFixed(3)),
      type: p.customerType, id: p.customerId,
    }));

    const rechargeBuckets: Record<number, { healthy: number; churn: number }> = {};
    filtered.forEach(p => {
      if (!rechargeBuckets[p.rechargeFreq]) rechargeBuckets[p.rechargeFreq] = { healthy: 0, churn: 0 };
      p.churn ? rechargeBuckets[p.rechargeFreq].churn++ : rechargeBuckets[p.rechargeFreq].healthy++;
    });
    const rechargeData = Object.entries(rechargeBuckets).sort(([a], [b]) => Number(a) - Number(b))
      .map(([freq, counts]) => ({ freq: `F${freq}`, ...counts }));

    const networkBuckets = [
      { label: "Poor", min: 0, max: 0.25 }, { label: "Fair", min: 0.25, max: 0.5 },
      { label: "Good", min: 0.5, max: 0.75 }, { label: "Excellent", min: 0.75, max: 1 },
    ].map(b => {
    ...
    const radarData = [
      ...
      { metric: "Recharge", value: Math.min((total ? filtered.reduce((s, p) => s + p.rechargeFreq, 0) / total : 0) * 10, 100) },
    ...
    const complaintTypes = filtered
      ...
      .reduce<Record<string, number>>((acc, p) => { acc[p.complaintText] = (acc[p.complaintText] || 0) + 1; return acc; }, {});
    const complaintData = Object.entries(complaintTypes).sort(([, a], [, b]) => b - a).slice(0, 6)
      .map(([name, value]) => ({ name, value }));

    return { total, churned, churnRate: total ? churned / total : 0, withComplaints, reactive, proactive, avgRisk, avgEngagement, avgNetwork, avgDataUsage, segmentDist, riskDist, aiActionDist, scatterData, rechargeData, networkBuckets, radarData, complaintData };
  }, [filtered]);
  ...
  return (
    <Box sx={{
      minHeight: "100vh", bgcolor: M.bg,
      fontFamily: "'Outfit', 'DM Sans', sans-serif",
      backgroundImage: `radial-gradient(ellipse 80% 30% at 50% -5%, ${M.yellow}14 0%, transparent 60%)`,
    }}>
      {/* ── Header ── */}
      <Box sx={{
        borderBottom: `1px solid ${M.border}`, px: { xs: 3, md: 5 }, py: 0,
        display: "flex", alignItems: "stretch", justifyContent: "space-between",
        position: "sticky", top: 0, zIndex: 100,
        bgcolor: M.surface, boxShadow: M.shadow,
      }}>
        {/* Yellow left bar */}
        <Stack direction="row" alignItems="center" spacing={2}>
          <Box sx={{ width: 5, bgcolor: M.yellow, alignSelf: "stretch" }} />
          <Stack direction="row" alignItems="center" spacing={1.5} sx={{ py: 2 }}>
            <Box sx={{ width: 36, height: 36, borderRadius: "9px", bgcolor: M.yellow, display: "flex", alignItems: "center", justifyContent: "center" }}>
              <SignalCellularAltIcon sx={{ color: M.black, fontSize: "1.15rem" }} />
            </Box>
            <Box>
              <Typography sx={{ fontWeight: 800, fontSize: "1rem", color: M.black, lineHeight: 1, letterSpacing: "-0.01em" }}>
        ...
        <Stack direction="row" alignItems="center" spacing={2} sx={{ pr: 1 }}>
          <Stack direction="row" alignItems="center" spacing={1}>
            <FiberSmartRecordIcon sx={{
              color: M.success, fontSize: "0.7rem",
              animation: "blink 2s ease-in-out infinite",
              "@keyframes blink": { "0%,100%": { opacity: 1 }, "50%": { opacity: 0.3 } },
            }} />
            <Typography sx={{ color: M.textMuted, fontSize: "0.72rem" }}>
              Live · {lastUpdated}
            </Typography>
            {isFetching && <CircularProgress size={12} sx={{ color: M.yellow }} />}
          </Stack>
          <Tooltip title="Refresh now">
            <IconButton onClick={() => refetch()} size="small" sx={{
              border: `1px solid ${M.border}`, borderRadius: "8px", color: M.textMuted,
              "&:hover": { color: M.black, bgcolor: M.yellowDim, borderColor: M.yellow },
            }}>
        ...
        <Stack direction={{ xs: "column", sm: "row" }} spacing={2} sx={{ mb: 4 }}>
          <FormControl size="small" sx={{ minWidth: 200 }}>
            ...
            <Select value={segment} label="Segment" onChange={e => setSegment(e.target.value)}
              sx={{ bgcolor: M.surface, borderRadius: "10px" }}>
          ...
          <Box sx={{ display: "flex", alignItems: "center", gap: 1, flexWrap: "wrap" }}>
            {["Healthy", "Silent Churn", "Vocal Churn"].map(seg => (
              <Chip key={seg} label={`${seg}: ${a.segmentDist.find(s => s.name === seg)?.value ?? 0}`} size="small"
                sx={{ bgcolor: `${SEGMENT_COLORS[seg]}15`, color: SEGMENT_COLORS[seg], border: `1px solid ${SEGMENT_COLORS[seg]}40`, fontWeight: 700, fontSize: "0.7rem" }} />
            ))}
          </Box>
        </Stack>

        {isLoading && <Box sx={{ display: "flex", justifyContent: "center", py: 10 }}><CircularProgress sx={{ color: M.yellow }} /></Box>}
        {isError && <Alert severity="error" sx={{ mb: 3 }}>Failed to load prediction data. Retrying…</Alert>}
            ...
            <Box>
              ...
              <Grid container spacing={2.5} sx={{ mb: 3 }}>
                <Grid size={{ xs: 12, sm: 6, md: 3 }}>
                  <StatCard icon={<PeopleIcon />} label="Total Customers" value={a.total} sub={`${data?.totalCount ?? 0} total in DB`} accent={M.info} />
                </Grid>
                <Grid size={{ xs: 12, sm: 6, md: 3 }}>
                  <StatCard icon={<TrendingDownIcon />} label="Churn Rate" value={pct(a.churnRate)} sub={`${a.churned} churned`} accent={M.danger} pulse={a.churnRate > 0.4} />
                </Grid>
                <Grid size={{ xs: 12, sm: 6, md: 3 }}>
                  <StatCard icon={<FlashOnIcon />} label="AI Interventions" value={a.reactive + a.proactive} sub={`${a.reactive} reactive · ${a.proactive} proactive`} accent={M.yellowDark} />
                </Grid>
                <Grid size={{ xs: 12, sm: 6, md: 3 }}>
                  <StatCard icon={<NetworkCheckIcon />} label="Avg Risk Score" value={fmt(a.avgRisk, 3)} sub={`Network: ${pct(a.avgNetwork)}`} accent={a.avgRisk > 0.6 ? M.danger : a.avgRisk > 0.3 ? M.warning : M.success} />
                </Grid>
              </Grid>
              ...
              <Grid container spacing={2.5} sx={{ mb: 4 }}>
                <Grid size={{ xs: 12, sm: 6, md: 3 }}>
                  <StatCard icon={<RecordVoiceOverIcon />} label="With Complaints" value={a.withComplaints} sub={`${pct(a.withComplaints / (a.total || 1))} of segment`} accent={M.purple} />
                </Grid>
                <Grid size={{ xs: 12, sm: 6, md: 3 }}>
                  <StatCard icon={<SupportAgentIcon />} label="Reactive Actions" value={a.reactive} sub="High risk + complaint" accent={M.danger} />
                </Grid>
                <Grid size={{ xs: 12, sm: 6, md: 3 }}>
                  <StatCard icon={<WarningAmberIcon />} label="Proactive Actions" value={a.proactive} sub="High risk, no complaint" accent={M.warning} />
                </Grid>
                <Grid size={{ xs: 12, sm: 6, md: 3 }}>
                  <StatCard icon={<SignalCellularAltIcon />} label="Avg Engagement" value={pct(a.avgEngagement)} sub={`Avg data: ${fmt(a.avgDataUsage)} GB`} accent={M.info} />
                </Grid>
              </Grid>
              ...
              <Grid container spacing={2.5} sx={{ mb: 4 }}>
                <Grid size={{ xs: 12, md: 4 }}>
                  <ChartCard title="Customer Segment Distribution" height={260}>
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie data={a.segmentDist} cx="50%" cy="50%" innerRadius={65} outerRadius={100} paddingAngle={4} dataKey="value">
                          {a.segmentDist.map((e, i) => <Cell key={i} fill={e.color} stroke="transparent" />)}
                        </Pie>
                        <RTooltip contentStyle={TOOLTIP} />
                        <Legend formatter={v => <span style={{ color: M.textSoft, fontSize: 12 }}>{v}</span>} />
                      </PieChart>
                    </ResponsiveContainer>
                  </ChartCard>
                </Grid>
                <Grid size={{ xs: 12, md: 4 }}>
                  <ChartCard title="Risk Level Distribution" height={260}>
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={a.riskDist} barSize={38}>
                        <CartesianGrid strokeDasharray="3 3" stroke={M.border} />
                        <XAxis dataKey="name" tick={{ fill: M.textMuted, fontSize: 11 }} axisLine={false} tickLine={false} />
                        <YAxis tick={{ fill: M.textMuted, fontSize: 11 }} axisLine={false} tickLine={false} />
                        <RTooltip contentStyle={TOOLTIP} />
                        <Bar dataKey="value" radius={[6, 6, 0, 0]}>
                          {a.riskDist.map((e, i) => <Cell key={i} fill={e.color} />)}
                        </Bar>
                      </BarChart>
                    </ResponsiveContainer>
                  </ChartCard>
                ...
                <Grid size={{ xs: 12, md: 4 }}>
                  <ChartCard title="AI Intervention Actions" height={260}>
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie data={a.aiActionDist} cx="50%" cy="50%" innerRadius={65} outerRadius={100} paddingAngle={4} dataKey="value">
                          {a.aiActionDist.map((e, i) => <Cell key={i} fill={e.color} stroke="transparent" />)}
                        </Pie>
                        <RTooltip contentStyle={TOOLTIP} />
                        <Legend formatter={v => <span style={{ color: M.textSoft, fontSize: 12 }}>{v}</span>} />
                      </PieChart>
              ...
              {/* ── Charts Row 2 ── */}
              <SectionTitle>Behavioural & Network Analytics</SectionTitle>
              <Grid container spacing={2.5} sx={{ mb: 4 }}>
                <Grid size={{ xs: 12, md: 6 }}>
                  <ChartCard title="Churn vs Healthy by Recharge Frequency" height={260}>
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={a.rechargeData} barSize={14} barGap={4}>
                        <CartesianGrid strokeDasharray="3 3" stroke={M.border} />
                        <XAxis dataKey="freq" tick={{ fill: M.textMuted, fontSize: 11 }} axisLine={false} tickLine={false} />
                        <YAxis tick={{ fill: M.textMuted, fontSize: 11 }} axisLine={false} tickLine={false} />
                        <RTooltip contentStyle={TOOLTIP} />
                        <Legend formatter={v => <span style={{ color: M.textSoft, fontSize: 12 }}>{v}</span>} />
                        <Bar dataKey="healthy" name="Healthy" fill={M.success} radius={[4, 4, 0, 0]} />
                        <Bar dataKey="churn" name="Churned" fill={M.danger} radius={[4, 4, 0, 0]} />
                      </BarChart>
                    </ResponsiveContainer>
                  </ChartCard>
                </Grid>
                <Grid size={{ xs: 12, md: 6 }}>
                  <ChartCard title="Network Quality → Avg Data Usage & Risk" height={260}>
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={a.networkBuckets} barSize={20} barGap={6}>
                        <CartesianGrid strokeDasharray="3 3" stroke={M.border} />
                        <XAxis dataKey="label" tick={{ fill: M.textMuted, fontSize: 11 }} axisLine={false} tickLine={false} />
                        <YAxis yAxisId="u" tick={{ fill: M.textMuted, fontSize: 11 }} axisLine={false} tickLine={false} />
                        <YAxis yAxisId="r" orientation="right" tick={{ fill: M.textMuted, fontSize: 11 }} axisLine={false} tickLine={false} domain={[0, 1]} />
                        <RTooltip contentStyle={TOOLTIP} formatter={(v: number, n: string) => [v.toFixed(3), n]} />
                        <Legend formatter={v => <span style={{ color: M.textSoft, fontSize: 12 }}>{v}</span>} />
                        <Bar yAxisId="u" dataKey="avgUsage" name="Avg Data (GB)" fill={M.info} radius={[4, 4, 0, 0]} />
                        <Bar yAxisId="r" dataKey="avgRisk" name="Avg Risk Score" fill={M.danger} radius={[4, 4, 0, 0]} />
                      </BarChart>
                    </ResponsiveContainer>
                  </ChartCard>
                </Grid>
              ...
              <Grid container spacing={2.5} sx={{ mb: 4 }}>
                <Grid size={{ xs: 12, md: 5 }}>
                  <ChartCard title="Engagement Score vs Churn Risk" height={280}>
                    <ResponsiveContainer width="100%" height="100%">
                      <ScatterChart>
                        <CartesianGrid strokeDasharray="3 3" stroke={M.border} />
                        <XAxis dataKey="x" name="Engagement" type="number" domain={[0, 1]} tick={{ fill: M.textMuted, fontSize: 10 }} axisLine={false} label={{ value: "Engagement", position: "insideBottom", offset: -2, fill: M.textMuted, fontSize: 10 }} />
                        <YAxis dataKey="y" name="Churn Risk" type="number" domain={[0, 1]} tick={{ fill: M.textMuted, fontSize: 10 }} axisLine={false} label={{ value: "Risk", angle: -90, position: "insideLeft", fill: M.textMuted, fontSize: 10 }} />
                        <RTooltip contentStyle={TOOLTIP} cursor={{ strokeDasharray: "3 3", stroke: M.border }} />
                        {["Healthy", "Silent Churn", "Vocal Churn"].map(type => (
                          <Scatter key={type} name={type} data={a.scatterData.filter(d => d.type === type)} fill={SEGMENT_COLORS[type]} opacity={0.8} />
                        ))}
                        <Legend formatter={v => <span style={{ color: M.textSoft, fontSize: 11 }}>{v}</span>} />
                      </ScatterChart>
                    </ResponsiveContainer>
                  </ChartCard>
                </Grid>
                <Grid size={{ xs: 12, md: 3 }}>
                  <ChartCard title="Customer Health Radar" height={280}>
                    <ResponsiveContainer width="100%" height="100%">
                      <RadarChart data={a.radarData}>
                        <PolarGrid stroke={M.border} />
                        <PolarAngleAxis dataKey="metric" tick={{ fill: M.textMuted, fontSize: 10 }} />
                        <PolarRadiusAxis domain={[0, 100]} tick={{ fill: M.textMuted, fontSize: 9 }} />
                        <Radar name="Score" dataKey="value" stroke={M.yellowDark} fill={M.yellow} fillOpacity={0.25} dot={{ fill: M.yellowDark, r: 3 }} />
                        <RTooltip contentStyle={TOOLTIP} formatter={(v: number) => [`${v.toFixed(1)}%`]} />
                      </RadarChart>
                    </ResponsiveContainer>
                  </ChartCard>
                </Grid>
                <Grid size={{ xs: 12, md: 4 }}>
                  <ChartCard title="Top Complaint Types" height={280}>
                    {a.complaintData.length === 0 ? (
                      <Box sx={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100%" }}>
                        <Typography sx={{ color: M.textMuted, fontSize: "0.8rem" }}>No complaints in this segment</Typography>
                      </Box>
                    ) : (
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={a.complaintData} layout="vertical" barSize={14}>
                          <CartesianGrid strokeDasharray="3 3" stroke={M.border} horizontal={false} />
                          <XAxis type="number" tick={{ fill: M.textMuted, fontSize: 10 }} axisLine={false} tickLine={false} />
                          <YAxis type="category" dataKey="name" tick={{ fill: M.textMuted, fontSize: 10 }} axisLine={false} tickLine={false} width={110} />
                          <RTooltip contentStyle={TOOLTIP} />
                          <Bar dataKey="value" fill={M.yellow} radius={[0, 6, 6, 0]} />
                        </BarChart>
                      </ResponsiveContainer>
                    )}
                  </ChartCard>
              ...
              <Grid container spacing={2.5} sx={{ mb: 4 }}>
                {["Healthy", "Silent Churn", "Vocal Churn"].map(seg => {
                  const sd = filtered.filter(p => p.customerType === seg);
                  const avgR = sd.length ? sd.reduce((s, p) => s + p.churnRiskScore, 0) / sd.length : 0;
                  const avgE = sd.length ? sd.reduce((s, p) => s + p.engagementScore, 0) / sd.length : 0;
                  const avgN = sd.length ? sd.reduce((s, p) => s + p.networkQuality, 0) / sd.length : 0;
                  const color = SEGMENT_COLORS[seg];
                  return (
                    <Grid size={{ xs: 12, md: 4 }} key={seg}>
                      <Card sx={{ bgcolor: M.surface, border: `1px solid ${M.border}`, borderLeft: `4px solid ${color}`, borderRadius: "14px", p: 3, boxShadow: M.shadow }}>
                        <Stack direction="row" alignItems="center" spacing={1.5} sx={{ mb: 2.5 }}>
                          <Box sx={{ width: 10, height: 10, borderRadius: "50%", bgcolor: color }} />
                          <Typography sx={{ fontWeight: 700, color: M.text, fontSize: "0.9rem" }}>{seg}</Typography>
                          <Chip label={sd.length} size="small" sx={{ bgcolor: `${color}15`, color, fontSize: "0.7rem", height: 20, fontWeight: 700 }} />
                        </Stack>
                        {[
                          { label: "Avg Churn Risk", value: avgR, color: avgR > 0.6 ? M.danger : avgR > 0.3 ? M.warning : M.success },
                          { label: "Avg Engagement", value: avgE, color: M.info },
                          { label: "Network Quality", value: avgN, color: M.purple },
                        ].map(m => (
                          <Box key={m.label} sx={{ mb: 1.5 }}>
                            <Stack direction="row" justifyContent="space-between" sx={{ mb: 0.5 }}>
                              <Typography sx={{ color: M.textMuted, fontSize: "0.72rem" }}>{m.label}</Typography>
              ...
              <Card sx={{ bgcolor: M.surface, border: `1px solid ${M.border}`, borderRadius: "14px", overflow: "hidden", boxShadow: M.shadow }}>
                <TableContainer sx={{ maxHeight: 420 }}>
                  <Table stickyHeader size="small">
                    <TableHead>
                      <TableRow>
                        {["Customer ID", "Segment", "Churn Risk", "Engagement", "Network", "Recharge Freq", "Complaint", "AI Action"].map(h => (
                          <TableCell key={h} sx={{ bgcolor: M.surfaceAlt, color: M.textMuted, fontSize: "0.65rem", fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", borderBottom: `2px solid ${M.border}` }}>
                            {h}
                          </TableCell>
                        ))}
                      </TableRow>
                    ...
                    <TableBody>
                      {filtered.slice(0, 20).map(p => {
                        ...
                        const riskColor = { low: M.success, medium: M.warning, high: M.danger }[riskLvl];
                        const actionColor = action === "Reactive" ? M.danger : action === "Proactive" ? M.warning : M.info;
                        return (
                          <TableRow key={p.id} sx={{ "&:hover": { bgcolor: M.yellowDim }, "& td": { borderBottom: `1px solid ${M.border}` } }}>
                            <TableCell sx={{ color: M.textSoft, fontSize: "0.8rem", fontWeight: 600 }}>#{p.customerId}</TableCell>
                            <TableCell>
                            ...
                            <TableCell>
                              <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                                <Box sx={{ width: 40, height: 4, borderRadius: 2, bgcolor: `${riskColor}20`, overflow: "hidden" }}>
                                  <Box sx={{ width: `${p.churnRiskScore * 100}%`, height: "100%", bgcolor: riskColor }} />
                                </Box>
                                <Typography sx={{ color: riskColor, fontSize: "0.75rem", fontWeight: 700 }}>{fmt(p.churnRiskScore, 3)}</Typography>
                              </Box>
                            </TableCell>
                            <TableCell sx={{ color: M.textMuted, fontSize: "0.75rem" }}>{pct(p.engagementScore)}</TableCell>
                            <TableCell sx={{ color: M.textMuted, fontSize: "0.75rem" }}>{pct(p.networkQuality)}</TableCell>
                            <TableCell sx={{ color: M.textMuted, fontSize: "0.75rem" }}>{p.rechargeFreq}x</TableCell>
                            <TableCell sx={{ color: M.textMuted, fontSize: "0.72rem", maxWidth: 120, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                              {p.complaintText === "None" ? <span style={{ color: M.border }}>—</span> : p.complaintText}
                            </TableCell>
                            <TableCell>
                              <Chip label={action} size="small" sx={{ bgcolor: `${actionColor}15`, color: actionColor, fontSize: "0.68rem", height: 20, fontWeight: 700, border: `1px solid ${actionColor}30` }} />
                            </TableCell>
                          </TableRow>
                        );
                      })}
              ...
              {/* Footer */}
              <Box sx={{ mt: 4, pt: 3, borderTop: `1px solid ${M.border}`, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 1 }}>
                <Typography sx={{ color: M.textFaint, fontSize: "0.7rem" }}>
                  Customer Insight Analysis · Auto-refreshes every 30s · Page {data?.page} of {data?.totalPages}
                </Typography>
                <Box sx={{ px: 2, py: 0.5, bgcolor: M.yellow, borderRadius: "6px" }}>
                  <Typography sx={{ color: M.black, fontSize: "0.65rem", fontWeight: 800, letterSpacing: "0.06em" }}>MTN NIGERIA</Typography>
                </Box>
              </Box>
            </Box>
          </Fade>
