...
const SENTIMENT_CONFIG: Record<string, { color: string; icon: React.ReactNode; label: string }> = {
  angry: { color: T.rose, icon: <SentimentVeryDissatisfiedIcon />, label: "Angry" },
  frustrated: { color: T.amber, icon: <SentimentDissatisfiedIcon />, label: "Frustrated" },
  neutral: { color: T.sky, icon: <SentimentNeutralIcon />, label: "Neutral" },
  positive: { color: T.green, icon: <SentimentSatisfiedAltIcon />, label: "Positive" },
};
      ...
      <CardContent sx={{ p: "20px 24px" }}>
        <Stack direction="row" justifyContent="space-between" alignItems="flex-start">
          <Box sx={{ flex: 1 }}>
            ...
            {sub && (
              <Typography sx={{ color: T.textMuted, fontSize: "0.72rem", mt: 0.75 }}>
                {sub}
              </Typography>
            )}
          </Box>
        ...
        return (
          <Box
            ...
            sx={{
              ...
              height: 10,
              borderRadius: "2px",
              bgcolor: filled ? T.gold : partial ? T.goldSoft : T.border,
              opacity: filled ? 1 : partial ? 0.6 : 0.3,
            }}
          />
        );
      })}
      <Typography sx={{ color: T.gold, fontSize: "0.78rem", fontWeight: 700, ml: 0.5 }}>
        {fmt(value)}
      </Typography>
    </Stack>
  );
...
const DailyTooltip = ({ active, payload, label }: any) => {
  ...
  return (
    ...
    >
      <Typography sx={{ color: T.textMuted, fontSize: "0.68rem", mb: 0.5 }}>
        {label}
      </Typography>
      <Typography sx={{ color: T.gold, fontSize: "1rem", fontWeight: 700 }}>
...
export default function CallAnalyticsDashboard() {
  ...
  const sentimentData = useMemo(() => {
    ...
    return Object.entries(d.callsBySentiment).map(([key, val]) => ({
      ...
      color: SENTIMENT_CONFIG[key]?.color ?? T.textMuted,
    }));
  }, [d]);
  ...
  const categoryData = useMemo(() => {
    ...
    return Object.entries(d.callsByCategory)
      .map(([key, val]) => ({
        ...
        color: CATEGORY_COLORS[key] ?? T.textMuted,
  ...
  const resolutionRadial = useMemo(() => {
    if (!d) return [];
    return [
      { name: "Resolved", value: d.resolutionRate, fill: T.green },
      { name: "Pending", value: 100 - d.resolutionRate, fill: T.border },
    ];
  }, [d]);
    ...
    >
      ...
      >
        <Stack direction="row" alignItems="center" spacing={2}>
          ...
          >
            <HeadsetMicIcon sx={{ color: T.gold, fontSize: "1.2rem" }} />
          </Box>
          <Box>
            <Typography sx={{ fontWeight: 800, fontSize: "1.05rem", color: T.text, lineHeight: 1, letterSpacing: "-0.01em" }}>
              CallPulse
            </Typography>
            <Typography sx={{ color: T.textMuted, fontSize: "0.62rem", letterSpacing: "0.14em" }}>
        ...
        <Stack direction="row" alignItems="center" spacing={2}>
          ...
          <ToggleButtonGroup
            ...
            sx={{
              ...
              "& .MuiToggleButton-root": {
                ...
                "&:hover": { bgcolor: T.surfaceHover, color: T.text },
              },
            }}
          >
            {DAY_OPTIONS.map((d) => (
          ...
          <Stack direction="row" alignItems="center" spacing={1}>
            {isFetching && <CircularProgress size={12} sx={{ color: T.gold }} />}
            <Typography sx={{ color: T.textFaint, fontSize: "0.72rem" }}>
              Last {days} days
            </Typography>
          </Stack>
          ...
          <Tooltip title="Refresh">
            <IconButton
              ...
              sx={{
                ...
                borderRadius: "8px",
                color: T.textMuted,
                "&:hover": { color: T.gold, borderColor: T.gold },
              }}
            >
      ...
      <Box sx={{ px: { xs: 3, md: 5 }, py: 4 }}>
        {isLoading && (
          <Box sx={{ display: "flex", justifyContent: "center", alignItems: "center", minHeight: "60vh" }}>
            <Stack alignItems="center" spacing={2}>
              ...
              <Typography sx={{ color: T.textMuted, fontSize: "0.8rem" }}>Loading analytics…</Typography>
            </Stack>
          </Box>
        )}
        ...
        {isError && (
          <Alert
            severity="error"
            sx={{ mb: 3, bgcolor: `${T.rose}15`, color: T.rose, border: `1px solid ${T.rose}33`, borderRadius: "10px" }}
          >
        ...
        {!isLoading && d && (
          <Fade in timeout={500}>
            <Box>
              ...
              <Grid container spacing={2} sx={{ mb: 4 }}>
                <Grid size={{ xs: 12, sm: 6, md: 3 }}>
                  <KpiCard
                    ...
                    sub="of total issues"
                    accent={d.resolutionRate >= 50 ? T.green : T.rose}
                  />
                </Grid>
                <Grid size={{ xs: 12, sm: 6, md: 3 }}>
                  <KpiCard
              ...
              <Grid container spacing={2} sx={{ mb: 4 }}>
                <Grid size={{ xs: 12 }}>
                  <ChartShell title={`Daily Call Volume — Last ${days} Days`} height={220} accent={T.gold}>
              ...
              {peakDay && (
                ...
                >
                  <TrendingUpIcon sx={{ color: T.gold, fontSize: "1.1rem" }} />
                  <Typography sx={{ color: T.text, fontSize: "0.82rem" }}>
                    <strong style={{ color: T.gold }}>Peak day:</strong> {peakDay.date} with{" "}
                    <strong style={{ color: T.gold }}>{peakDay.calls} calls</strong> —{" "}
              ...
              <Grid container spacing={2} sx={{ mb: 4 }}>
                ...
                <Grid size={{ xs: 12, md: 5 }}>
                  <ChartShell title="Calls by Category" height={280} accent={T.teal}>
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={categoryData} layout="vertical" barSize={16} margin={{ left: 10, right: 30 }}>
                        <CartesianGrid strokeDasharray="3 3" stroke={T.border} horizontal={false} />
                        <XAxis type="number" tick={{ fill: T.textMuted, fontSize: 10 }} axisLine={false} tickLine={false} allowDecimals={false} />
                        <YAxis type="category" dataKey="name" tick={{ fill: T.textMuted, fontSize: 11 }} axisLine={false} tickLine={false} width={60} />
                        <RechartsTooltip
                ...
                <Grid size={{ xs: 12, md: 4 }}>
                  <ChartShell title="Customer Sentiment" height={280} accent={T.amber}>
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie
                          data={sentimentData}
                        ...
                        <Box key={s.name}>
                          <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 0.5 }}>
                            <Stack direction="row" alignItems="center" spacing={0.8}>
                              ...
                              <Typography sx={{ color: T.text, fontSize: "0.78rem", fontWeight: 600 }}>
                                {s.name}
                              </Typography>
                            </Stack>
                ...
                <Grid size={{ xs: 12, md: 4 }}>
                  ...
                  >
                    ...
                    <Box sx={{ position: "relative", width: 180, height: 180 }}>
                      ...
                      >
                        <Typography
                          sx={{ color: T.green, fontSize: "1.8rem", fontWeight: 800, lineHeight: 1 }}
                        >
                          {fmt(d.resolutionRate)}%
                        </Typography>
                        <Typography sx={{ color: T.textMuted, fontSize: "0.68rem" }}>resolved</Typography>
                      </Box>
                    </Box>
                    <Stack direction="row" spacing={2.5} sx={{ mt: 2 }}>
                      <Stack alignItems="center">
                        <Typography sx={{ color: T.green, fontWeight: 700, fontSize: "1.1rem" }}>
                          {d.issuesResolved}
                        </Typography>
                        <Typography sx={{ color: T.textMuted, fontSize: "0.68rem" }}>Resolved</Typography>
                      </Stack>
                      ...
                      <Stack alignItems="center">
                        <Typography sx={{ color: T.rose, fontWeight: 700, fontSize: "1.1rem" }}>
                          {d.issuesPending}
                        </Typography>
                        <Typography sx={{ color: T.textMuted, fontSize: "0.68rem" }}>Pending</Typography>
                      </Stack>
                    </Stack>
                  </Card>
                </Grid>
                ...
                <Grid size={{ xs: 12, md: 4 }}>
                  ...
                  >
                    ...
                    >
                      ...
                      <Typography component="span" sx={{ color: T.textFaint, fontSize: "1.5rem", fontWeight: 400 }}>
                        /5
                      </Typography>
                    </Typography>
                    ...
                    <Box sx={{ mt: 3 }}>
                      {[5, 4, 3, 2, 1].map((star) => {
                        const width = star === Math.round(d.averageSatisfactionRating) ? 70 : star < Math.round(d.averageSatisfactionRating) ? 40 : 15;
                        return (
                          <Stack key={star} direction="row" alignItems="center" spacing={1} sx={{ mb: 0.7 }}>
                            <Typography sx={{ color: T.textMuted, fontSize: "0.68rem", width: 12 }}>{star}</Typography>
                            <Box
                              sx={{
                                height: 5,
                                width: `${width}%`,
                                bgcolor: star >= 4 ? T.gold : star === 3 ? T.amber : T.rose,
                                borderRadius: 3,
                ...
                <Grid size={{ xs: 12, md: 4 }}>
                  ...
                  >
                    ...
                    {Object.keys(d.unresolvedByCategory ?? {}).length === 0 ? (
                      ...
                      >
                        <CheckCircleOutlineIcon sx={{ color: T.green, fontSize: "2.5rem" }} />
                        <Typography sx={{ color: T.green, fontWeight: 700, fontSize: "0.9rem" }}>
                          All clear!
                        </Typography>
                      ...
                      <Stack spacing={2}>
                          ...
                          <Box key={cat}>
                              ...
                              <Stack direction="row" alignItems="center" spacing={1}>
                                <Box
                                  sx={{
                                    ...
                                    height: 8,
                                    borderRadius: "2px",
                                    bgcolor: CATEGORY_COLORS[cat] ?? T.textMuted,
                                  }}
                                />
                                <Typography sx={{ color: T.text, fontSize: "0.82rem", fontWeight: 600 }}>
                                  {cap(cat)}
                                </Typography>
                              </Stack>
                              <Chip
                            ...
                            <LinearProgress
                              ...
                              sx={{
                                ...
                                borderRadius: 3,
                                bgcolor: T.border,
                                "& .MuiLinearProgress-bar": { bgcolor: T.rose, borderRadius: 3 },
                              }}
                            />
                          </Box>
                        ))}
                        ...
                        <Box sx={{ mt: 1, pt: 2, borderTop: `1px solid ${T.border}` }}>
                          <Typography sx={{ color: T.textMuted, fontSize: "0.68rem", mb: 1.5, letterSpacing: "0.08em" }}>
                            ALL CATEGORY VOLUMES
                          </Typography>
                          {categoryData.map((cat) => (
                            <Stack key={cat.name} direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 0.8 }}>
                              <Stack direction="row" alignItems="center" spacing={0.8}>
                                <Box sx={{ width: 6, height: 6, borderRadius: "50%", bgcolor: cat.color }} />
                                <Typography sx={{ color: T.textMuted, fontSize: "0.72rem" }}>{cat.name}</Typography>
                              </Stack>
              ...
              >
                {[
                  { label: "Calls / Client", value: fmt(d.totalCalls / Math.max(d.totalUniqueClients, 1), 1), color: T.teal },
                  { label: "Avg Duration", value: `${fmt(d.averageCallDurationMinutes, 1)} min`, color: T.sky },
                  { label: "Active Rate", value: `${fmt((d.activeCalls / Math.max(d.totalCalls, 1)) * 100)}%`, color: T.amber },
                  { label: "Issue Rate", value: `${fmt(((d.issuesResolved + d.issuesPending) / Math.max(d.totalCalls, 1)) * 100)}%`, color: T.rose },
                  { label: "Satisfaction", value: `${fmt(d.averageSatisfactionRating)} / 5`, color: T.gold },
                ].map((m) => (
                  <Box key={m.label} sx={{ textAlign: "center" }}>
                    <Typography sx={{ color: m.color, fontWeight: 800, fontSize: "1.3rem", lineHeight: 1 }}>
                      {m.value}
                    </Typography>
                    <Typography sx={{ color: T.textMuted, fontSize: "0.65rem", letterSpacing: "0.08em" }}>
                      {m.label}
                    </Typography>
                  </Box>
                ))}
                <Typography sx={{ color: T.textFaint, fontSize: "0.68rem", ml: "auto" }}>
